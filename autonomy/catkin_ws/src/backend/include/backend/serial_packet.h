...
public:
	...
	virtual int read(void) =0; // For Arduino-style read
	virtual size_t read(uint8_t *buffer, size_t size) =0; // For ROS-style read
...
/* Packet receive */
	...
	*/
	int read_packet(A_packet &p) {
		if (serial.available() == 0) return 0; // no data to read
		uint8_t c;
		#ifndef __AVR
			int success = serial.read(&c, 1); // else read next byte
			if (!success) return 0; // (hmmm, why did available return true then?)
		#else
			c = serial.read();
