...
//float dgain = 0;
//
////speed_controller_t<NUM_AVERAGES> encoder_mining_left(pgain, igain, dgain, encoder_bus_2[3], 80, motor_mining_left); // Mining head left side motor
////speed_controller_t<NUM_AVERAGES> encoder_mining_right(pgain, igain, dgain, encoder_bus_2[3], 80, motor_mining_right); // Mining head right side motor
//
//speed_controller_t<NUM_AVERAGES> encoder_R(pgain, igain, dgain,encoder_bus_1[6],80,motor_D); // Encoder for roll motor
//speed_controller_t<NUM_AVERAGES> encoder_DL1(pgain, igain, dgain, encoder_bus_2[5],13,motor_L);  //Left front wheel encoder
//speed_controller_t<NUM_AVERAGES> encoder_DL2(pgain, igain, dgain, encoder_bus_2[8],13,motor_L);  //Left back wheel encoder
//speed_controller_t<NUM_AVERAGES> encoder_DR1(pgain, igain, dgain, encoder_bus_2[4],13,motor_R); //Right front wheel encoder
//speed_controller_t<NUM_AVERAGES> encoder_DR2(pgain, igain, dgain, encoder_bus_2[7],13,motor_R);  //Right back wheel encoder
//
//encoder_t limit_top(encoder_bus_1[3]);
//encoder_t limit_bottom(encoder_bus_1[5]);
//
...
//  int mine=robot.power.mine;
//  //mine=encoder_M.update(mine,robot.power.mineMode!=0||robot.power.mineDump!=0);
////  int mine_left=encoder_mining_left.update(mine,robot.power.torqueControl==0,24);
////  int mine_right=encoder_mining_right.update(mine,robot.power.torqueControl==0,24);
// 
////  send_motor_power(mine, motor_mining_left, encoder_mining_left);
////  send_motor_power(mine, motor_mining_right, encoder_mining_right);
//
//  motor_F.drive(robot.power.dump);
//  motor_front_linear.drive(robot.power.head_extend);
//}
...
//
//milli_t last_milli=0;
//uint16_t count_mono_mining_left_old=0;
//uint16_t count_mono_mining_right_old=0;
//
//void low_latency_ops() {
//  unsigned long micro=micros();
//  milli=micro>>10; // approximately == milliseconds
...
//  
//
//  //Encoder for left mining motor
////  encoder_mining_left.read();
//  // robot.sensor.Mspeed=encoder_M.period;
////  robot.sensor.Mcount_left=encoder_mining_left.count_mono;
//
//  // Encoder for right mining motor
////  encoder_mining_right.read();
////  robot.sensor.Mcount_right=encoder_mining_right.count_mono;
//
//  // For each mining encoder, get number of ticks since last update
////  uint16_t delta_ticks_mining_left = encoder_mining_left.count_mono - count_mono_mining_left_old;
////  uint16_t delta_ticks_mining_right = encoder_mining_right.count_mono - count_mono_mining_right_old;
//
//  //
////  if(delta_ticks_mining_left == 0) 
////  {
//////    robot.sensor.Mcount = encoder_mining_left.count_mono;
////  }
////  else if(delta_ticks_mining_right == 0) 
////  {
////    robot.sensor.Mcount = encoder_mining_right.count_mono;
////  }
////  else
////  {
//////    robot.sensor.Mcount = (encoder_mining_left.count_mono + encoder_mining_right.count_mono)/2;
////  }
//
//  // Store old encoder ticks for next update
////  count_mono_mining_left_old = encoder_mining_left.count_mono;
////  count_mono_mining_right_old = encoder_mining_right.count_mono; 
//  
//
//
//  // Fuse the encoder ticks for the two sides
////  if(!(encoder_mining_left.count_mono) || !(encoder_mining_right.count_mono)))
////  {
//    // Report the average of the two encoders
////    robot.sensor.Mcount = (encoder_mining_left.count_mono + encoder_mining_right.count_mono)/2;
////  }
//
//  //Encoder for roll motor
